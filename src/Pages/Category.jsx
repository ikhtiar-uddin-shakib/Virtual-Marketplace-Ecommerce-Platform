...
const Category = () => {
    ...
    console.log("Loaded data in Category:", shoppingMalls); // Debug logging
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 container mx-auto my-20">
                {
                    shoppingMalls.map((mall, index) => (
                        <div key={index} className="max-w-2xl bg-blue-50 rounded-lg shadow-md dark:bg-gray-800 z-0">
                            <img src={mall.photo} className="object-cover w-full h-64" alt={mall.name} />
                            <div className="p-6">
                                <div>
                                ...
                                {mall.floors && mall.floors.length > 0 && (
                                    <details className="dropdown container mx-auto my-4">
                                        ...
                                        <ul className="p-2 shadow menu dropdown-content z-[1] bg-pink-100 rounded-box w-52">
                                            {
                                                mall.floors.map((floor, idx) => (
                                                    <Link key={idx} to={`/floor/${mall.id}/${floor.num}`}>
                                                        <li><a className="font-semibold hover:text-blue-800">Floor : {floor.num}</a></li>
                                                    </Link>
                                                ))
                                            }
                                        </ul>
